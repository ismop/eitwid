package pl.ismop.web.client.widgets.old.maps.google;
...
//		eventBus.areaSelected(top, left, bottom, right);
	...
	private native void updateSectionOnMap(String leveeId) /*-{
		var profileData = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sectionMapData;
		...
		if(foundFeature) {
			var color = thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::getFeatureColor(Ljava/lang/String;)(foundFeature.getId());
	...
	private native void selectSensor(String sensorId, boolean show) /*-{
		var sensorData = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sensorMapData;
		var foundFeature = null;
	...
	private native JavaScriptObject showMap(Object bounds) /*-{
		var map = new $wnd.google.maps.Map($doc.getElementById(this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::elementId), {
		...
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sectionMapData = sectionData;
		sectionData.loadGeoJson($wnd.profileUrl);
		sectionData.setMap(map);
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::overrideStyle(Lcom/google/gwt/core/client/JavaScriptObject;)(sectionData);
		
		var sensorData = new $wnd.google.maps.Data();
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sensorMapData = sensorData;
		sensorData.loadGeoJson($wnd.sensorUrl);
		//sensors are not shown at the start
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::overrideStyle(Lcom/google/gwt/core/client/JavaScriptObject;)(sensorData);
		
		var profileData = new $wnd.google.maps.Data();
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData = profileData;
		profileData.setMap(map);
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::overrideStyle(Lcom/google/gwt/core/client/JavaScriptObject;)(profileData);
...
//			thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::onAreaSelected(FFFF)(r.getBounds().getNorthEast().lat(),
...
//		});
	...
	private native void overrideStyle(JavaScriptObject mapData) /*-{
		...
		mapData.setStyle(function(feature) {
			return {
				strokeColor: thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::getFeatureStrokeColor(Ljava/lang/String;)(feature.getId()),
				fillOpacity: 0.9,
				strokeOpacity: 1.0,
				fillColor: thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::getFeatureColor(Ljava/lang/String;)(feature.getId()),
				strokeWeight: thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::getFeatureStrokeWeight(Ljava/lang/String;)(feature.getId()),
				icon: thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::getFeatureIcon(Ljava/lang/String;)(feature.getId())
			};
		});
		...
		mapData.addListener('click', function(event) {
			thisObject.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::onFeatureClicked(Ljava/lang/String;)(event.feature.getId());
		});
	}-*/;
	
	private native String getFeatureType(String featureId) /*-{
		if(featureId) {
			var feature = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sectionMapData.getFeatureById(featureId);
			
			if(feature == null) {
				feature = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::sensorMapData.getFeatureById(featureId);
			}
			
			if(feature == null) {
				feature = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.getFeatureById(featureId);
			}
	...
	private native String getProfileId(String featureId) /*-{
		if(featureId) {
			var profileData = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData;
			var feature = profileData.getFeatureById(featureId);
	...
	private native void updateDygraphData(String data) /*-{
		var graph = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::currentGraph;
		
		if(graph) {
			this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::currentGraph.updateOptions({
				file: data
			});
		}
	}-*/;
	...
	private native void showLayer(JavaScriptObject mapData, boolean show) /*-{
		if(show) {
			mapData.setMap(this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::map);
		} else {
			mapData.setMap(null);
		}
	}-*/;
	...
	private native void panMap(Object bounds) /*-{
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::map.fitBounds(bounds);
	}-*/;
	
	private native void addFeatures(String jsonValue) /*-{
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.addGeoJson(JSON.parse(jsonValue));
	}-*/;
	
	private native void removeProfileFeature(String profileFeatureId) /*-{
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.remove(
				this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.getFeatureById(profileFeatureId));
	}-*/;
	
	private native void removeAggregateFeature(String aggregateFeatureId) /*-{
		this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.remove(
				this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData.getFeatureById(aggregateFeatureId));
	}-*/;
	
	private native String getAggregateId(String featureId) /*-{
		if(featureId) {
			var aggregateData = this.@pl.ismop.web.client.widgets.old.maps.google.GoogleMapsPresenter::profileMapData;
